...
class BayesianMCMC:
    ...
    def build_model(self, draws_df: pd.DataFrame):
        ...
        with pm.Model() as model:
            # Dirichlet prior for probabilities (they sum to 1 automatically)
            alphas = pm.Gamma('alphas', alpha=2, beta=1, shape=self.max_number)
            probabilities = pm.Dirichlet('probabilities', a=alphas)
    ...
    def get_probabilities(self) -> np.ndarray:
        ...
        return probs / probs.sum()  # safety normalization
